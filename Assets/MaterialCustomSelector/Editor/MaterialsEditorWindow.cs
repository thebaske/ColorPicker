...
public class MaterialsEditorWindow : EditorWindow
{
        ...
        for (int i = 0; i < textures.Length; i++)
        {
            ...
            if (GUILayout.Button($"Set {i}", GUILayout.MaxWidth(70)))
            {
                if (Selection.activeGameObject != null)
                {
                    ...
                    float pixelValue = Utility.RemapValues(0, textures.Length - 1, 0.01f, 0.97f, i);
    ...
    private void PackTextures(TextureData[] datas)
    {
        ...
        {
            atlasColors[i] = datas[(int)Utility.RemapValues(0, atlasColors.Length, 0, datas.Length, i)].color;
        }
        atlas.SetPixels32(atlasColors);
        // Rect[] tmp = atlas.PackTextures(texArray, 0, 1024);
        AssetDatabase.CreateAsset(atlas, "Assets/ColorPicker_Atlas/" + "SharedAtlas" + ".Texture2D");
