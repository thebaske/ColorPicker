...
namespace RapidIconUIC
{
	...
	string[] filterNames = new string[] { "Prefabs & Models", "Prefabs Only", "Models Only" };
		...
		void DrawIcons(float gridWidth)
		{
			...
			List<Icon> visibleIcons = new List<Icon>();
			...
			foreach (Icon icon in objectIcons.Values)
			{
				if (!assetList.selectedFolders.Contains(icon.folderPath))
				{
					icon.selected = false;
		...
		Icon CreateIcon(ObjectPathPair objectPathPair)
		{
			Icon icon = new Icon(Shader.Find("RapidIcon/ObjectRender"), rapidIconRootFolder);
