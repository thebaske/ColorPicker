...
namespace RapidIconUIC
{
	...
	public class IconEditor
	{
		...
		void ExportIcon(Icon icon, bool inBatchExport)
		{
			...
			string fileName = icon.exportFolderPath + icon.exportName;
			...
			else
			{
				...
				foreach(Icon icon in assetGrid.selectedIcons)
				{
					if (icon != currentIcon)
					{
						...
						bool doUpdate = result == 2 | !(tab == 4 && icon.exportResolution == currentIcon.exportResolution);
		...
		void DrawPostProcessingControls()
		{
			...
			if (EditorGUI.EndChangeCheck())
			{
				Undo.RecordObject(window, "Change Filter Mode");
		...
		void DrawExportControls()
		{
			...
			EditorGUIUtility.labelWidth = 80;
			string tmp = EditorGUILayout.TextField("Export Name", currentIcon.exportName + ".png");
			EditorGUIUtility.labelWidth = tmpWdith;
			if (EditorGUI.EndChangeCheck())
			{
				if (tmp.Length >= 4 && tmp.Substring(tmp.Length - 4) == ".png")
					tmp = tmp.Substring(0, tmp.Length - 4);

				if (tmp.Length > 0)
				{
					currentIcon.exportName = tmp;
					currentIcon.saveData = true;
				}
			}
