...
namespace Kamgam.PivotCursorTool
{
    partial class PivotCursorTool
    {
        ...
        protected void drawOriginalCursorGizmo(Transform transform)
        {
            ...
            if (currentTool != PTool.Rotate && Vector3.SqrMagnitude(transform.position - cursorPosition) < 0.01f)
                return;
        ...
        void restoreCursorFromLocalPosition()
        {
            ...
            if (settings.UpdateCursorWithObject == PivotCursorToolSettings.UpdateCursorOptions.Never)
                return;

            // do not restore if the current rotation mode is global
            if (Tools.pivotRotation == PivotRotation.Global && settings.UpdateCursorWithObject == PivotCursorToolSettings.UpdateCursorOptions.OnlyIfLocal)
                return;
