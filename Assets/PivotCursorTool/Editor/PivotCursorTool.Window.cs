...
namespace Kamgam.PivotCursorTool
{
    ...
    partial class PivotCursorTool
    {
        ...
        void drawWindow(SceneView sceneView, int controlID)
        {
            ...
            cursorDetailsWindowRect = GUILayout.Window(controlID, cursorDetailsWindowRect, drawWindowContent, "Cursor (hold \"v\" to snap)");
        ...
        void drawWindowContent(int controlID)
        {
            ...
            selectedStyle.normal.background = BackgroundTexture.Get(new Color(0.5f, 0.5f, 0.5f));

            GUILayout.BeginHorizontal();

            bool toolIsActive = currentTool == PTool.Cursor;
            if (DrawButton("", icon: "d_ToolHandlePivot@2x", style: toolIsActive ? selectedStyle : null,
                tooltip: "Change the cursor"))
            {
                switchToolTo(PTool.Cursor);
            }

            toolIsActive = currentTool == PTool.Move;
            if (DrawButton("", icon: toolIsActive ? "d_MoveTool On@2x" : "d_MoveTool@2x", style: toolIsActive ? selectedStyle : null,
                tooltip: "Move"))
            {
            ...
            if (DrawButton("", icon: toolIsActive ? "d_RotateTool On@2x" : "d_RotateTool@2x", style: toolIsActive ? selectedStyle : null,
                tooltip: "Rotate around the cursor."))
            {
            ...
            if (DrawButton("",
                ...
                style: toolIsActive ? selectedStyle : null,
