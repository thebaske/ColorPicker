...
{
    [EditorTool("Cursor")]
    partial class PivotCursorTool : EditorTool, ISerializationCallbackReceiver
    {
        public enum PTool
        {
        ...
        public static PivotCursorTool Instance;
        ...
        /// The tool is actually a ScriptableObject which is Instantiate and deserialized by Unity.
        /// </summary>
        ...
        public static void Activate(PTool? tool = null)
        {
            ...
            ToolManager.SetActiveTool(typeof(PivotCursorTool));
#else
            EditorTools.SetActiveTool(typeof(PivotCursorTool));
#endif
        ...
        {
            if (PivotCursorActiveState.IsActive)
            {
                SceneView.lastActiveSceneView.Focus();
        ...
        public override void OnToolGUI(EditorWindow window)
        {
            ...
            // Therefore we need to call HandleKeyPressEvents manually from here.
            if (GlobalKeyEventHandler.RegistrationSucceeded)
            {
            ...
            try
            {
                ...
                if (current.type == EventType.KeyDown)
                {
                    ...
                    if (!IsModifierKeyPressed(current))
                    {
                        ...
                        {
                            useKey = true;
                            Log("Cursor: Sorry, the rect tool is not supported. Press Esc to return to normal tools.");
                        }
                        ...
                        else if (current.keyCode == settings.FrameSelectedKey)
                        {
                            ...
                            Log("Cursor: Default frame selection was replaced. Focusing cursor position now.");
                        }
                    }
                    ...
                    {
                        if (current.keyCode == KeyCode.Z && HasCursorUndoActions())
                        {
                            UndoCursor();
                            useKey = true;
                        }
                        else if (current.keyCode == KeyCode.Y && HasCursorRedoActions())
                        {
                            RedoCursor();
                            useKey = true;
                        }
