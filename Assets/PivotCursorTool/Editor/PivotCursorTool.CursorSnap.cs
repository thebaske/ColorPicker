...
namespace Kamgam.PivotCursorTool
{
    ...
    partial class PivotCursorTool
    {
        ...
        private void scanRendererForSnapVertices<TKey>(Renderer renderer, Camera cam, PivotCursorToolSettings settings,
            ...
            where TKey : Component
        {
            ...
            if (renderer != null && tmpSnapBounds.IntersectRay(ray))
            {
                ...
                if (verticesCache.ContainsKey(component))
                {
        ...
        void buildScenesRaycastCache()
        {
            ...
            else
            {
                ...
                spriteRenderers = Transform.FindObjectsOfType<SpriteRenderer>();
            }
            ...
            if (skinnedMeshRenderers != null)
            {
                ...
                {
                    if (/*!isLayerInLayers(meshRenderer.gameObject.layer, Tools.lockedLayers) && */ // Not sure if we want this. Wait for user feedback.
                           isLayerInLayers(meshRenderer.gameObject.layer, Tools.visibleLayers))
                    {
                        ...
                        // fast alternative(though without backing)
                        /*
                        var vertices = new List<Vector3>();
